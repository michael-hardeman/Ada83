...
PROCEDURE CD2A32E IS
     ...
     BEGIN
          ...
          FOR I IN IDENT (PI0) .. IDENT (PIO2) LOOP
               IF NOT (I IN PI0 .. PI2) OR
          ...
          IF INT'VAL (0)   /= IDENT (PI0)  OR
             INT'VAL (63) /= IDENT (PIO1) OR
             INT'VAL (126) /= IDENT (PIO2) THEN
          ...
          IF INT'PRED (PIO1) /= IDENT (62)  OR
             INT'PRED (PI2)  /= IDENT (125) THEN
          ...
          IF INT'VALUE ("0")   /= IDENT (PI0)  OR
             INT'VALUE ("63") /= IDENT (PIO1) OR
             INT'VALUE ("126") /= IDENT (PIO2) THEN
...
BEGIN
     ...
     IF NOT ((+INTARRAY(2) = INTARRAY(2))     AND
             (-INTARRAY(1) = -63)            AND
             (ABS INTARRAY(2) = INTARRAY(2))) THEN
          FAILED ("INCORRECT RESULTS FOR UNARY ARITHMETIC " &
                  "OPERATORS - 3");
     END IF;
     ...
     IF INT'VAL (0)   /= IDENT (INTARRAY(0)) OR
        INT'VAL (63) /= IDENT (INTARRAY(1)) OR
        INT'VAL (126) /= IDENT (INTARRAY(2)) THEN
     ...
     IF INT'PRED (INTARRAY(1)) /= IDENT (62)  OR
        INT'PRED (INTARRAY(2)) /= IDENT (125) THEN
     ...
     IF INT'VALUE ("0")   /= IDENT (INTARRAY(0)) OR
        INT'VALUE ("63") /= IDENT (INTARRAY(1)) OR
        INT'VALUE ("126") /= IDENT (INTARRAY(2)) THEN
     ...
     IF NOT (((IREC.COMP0 + IREC.COMP2) = IREC.COMP2)  AND
             ((IREC.COMP2 - IREC.COMP1) = IREC.COMP1)  AND
             ((IREC.COMP1 * IDENT (2))  = IREC.COMP2)  AND
             ((IREC.COMP2 / IREC.COMP1) = IDENT (2))   AND
             ((IREC.COMP1 ** 1)         = IDENT (63))  AND
             ((IREC.COMP2 REM 10)       = IDENT (6))   AND
             ((IREC.COMP1 MOD 10)       = IDENT (3)))  THEN
          FAILED ("INCORRECT RESULTS FOR BINARY ARITHMETIC " &
                  "OPERATORS - 4");
     END IF;
     ...
     IF INT'POS (IREC.COMP0) /= IDENT_INT (0)   OR
        INT'POS (IREC.COMP1) /= IDENT_INT (63) OR
        INT'POS (IREC.COMP2) /= IDENT_INT (126) THEN
     ...
     IF INT'SUCC (IREC.COMP0) /= IDENT (1)   OR
        INT'SUCC (IREC.COMP1) /= IDENT (64) THEN
     ...
     IF INT'IMAGE (IREC.COMP0) /= IDENT_STR (" 0")   OR
        INT'IMAGE (IREC.COMP1) /= IDENT_STR (" 63") OR
        INT'IMAGE (IREC.COMP2) /= IDENT_STR (" 126") THEN
