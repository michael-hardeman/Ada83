...
BEGIN
     ...
     CASE C IS
          ...
          WHEN '1' =>
               D64005FC (LEVEL'SUCC(L), IDENT_CHAR('1'), T);

          WHEN '2' =>
               D64005FB (L, IDENT_CHAR('3'), T);

          WHEN '3' =>
               D64005FC (LEVEL'SUCC(L), IDENT_CHAR('2'), T);
     END CASE;

     -- APPEND ALL L AND C TO T IN REVERSE ORDER.
     T.S (T.E+1 .. T.E+N) := D64005FB.L & D64005FB.C &
