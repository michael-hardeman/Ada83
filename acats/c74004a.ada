...
PROCEDURE C74004A IS
     ...
     BEGIN
          ...
          IF NOT EQUAL(VAL,18) THEN
               FAILED ("IMPROPER RESULT FROM 'MANTISSA");
          END IF;

          FVAL := FLT'EPSILON;

          IF FVAL /= 2.0**(1 - FLT'MANTISSA) THEN
               FAILED ("IMPROPER RESULT FROM 'EPSILON");
          END IF;

          VAL := FLT'EMAX;

          IF NOT EQUAL(VAL,72) THEN
               FAILED ("IMPROPER RESULT FROM 'EMAX");
          END IF;

          FVAL := FLT'SMALL;

          IF FVAL /= 2.0**(-FLT'EMAX - 1) THEN
               FAILED ("IMPROPER RESULT FROM 'SMALL");
          END IF;

          FVAL := FLT'LARGE;

          IF FVAL /= 2.0**FLT'EMAX*(1.0-2.0**(-FLT'MANTISSA)) THEN
               FAILED ("IMPROPER RESULT FROM 'LARGE");
          END IF;
          ...
          IF FVAL /= (2.0**FIX'MANTISSA - 1.0)*FIX'SMALL THEN
               FAILED ("IMPROPER RESULT FROM 'LARGE");
          END IF;
     END P;
