...
-- FORMAL TYPE), 'BASE, 'FIRST, 'LAST, 'SIZE, 'ADDRESS, 'DIGITS, 
-- 'MANTISSA, 'EPSILON, 'EMAX, 'SMALL, 'LARGE, 'SAFE_EMAX, 
-- 'SAFE_SMALL, 'SAFE_LARGE, 'MACHINE_RADIX, 'MACHINE_MANTISSA, 
-- 'MACHINE_EMAX, 'MACHINE_EMIN, 'MACHINE_ROUNDS, 'MACHINE_OVERFLOWS.
...
BEGIN
     ...
     DECLARE -- (C) CHECKS FOR ATTRIBUTES.
          ...
          BEGIN
               ...
               IF T'EMAX /= 4 * T'MANTISSA THEN
                    FAILED ( "INCORRECT VALUE FOR " & STR & "'EMAX" );
               END IF;

               IF T'EPSILON /= 2.0 ** (1 - T'MANTISSA) THEN
                    FAILED ( "INCORRECT VALUE FOR " & 
                              STR & "'EPSILON" );
               END IF;

               IF T'SMALL /= 2.0 ** (-T'EMAX - 1) THEN
                    FAILED ( "INCORRECT VALUE FOR " & STR & "'SMALL" );
               END IF;

               IF T'LARGE /= 
                  2.0 ** T'EMAX * (1.0 - 2.0 ** (-T'MANTISSA)) THEN
                    FAILED ( "INCORRECT VALUE FOR " & STR & "'LARGE" );
               END IF;
