...
PROCEDURE CD2A31C IS
     ...
     PRIVATE
          ...
          FOR PRIVATE_INT'SIZE USE SPECIFIED_SIZE;
...
-- SIZE SPECIFICATION GIVEN IN A GENERIC PROCEDURE.
     ...
     PROCEDURE GENPROC IS
          ...
          PROCEDURE CHECK_1 IS NEW LENGTH_CHECK (CHECK_INT);
     ...
     BEGIN
          ...
          END IF;
     CHECK_1 (-60, 9, "GENERIC CHECK_INT");
     ...
     PROCEDURE CHECK_1 IS NEW LENGTH_CHECK (CHECK_INT);
     PROCEDURE CHECK_2 IS NEW LENGTH_CHECK (DERIVED_INT);
     PROCEDURE CHECK_4 IS NEW LENGTH_CHECK (INT_IN_P);
     PROCEDURE CHECK_5 IS NEW LENGTH_CHECK (ALT_INT_IN_P);

BEGIN

     TEST("CD2A31C", "'SIZE SPECIFICATIONS CAN BE GIVEN IN VISIBLE " &
                     "OR PRIVATE PART OF PACKAGE FOR INTEGER " &
                     "TYPE DECLARED IN VISIBLE PART, AND FOR " &
                     "DERIVED INTEGER " &
                     "TYPES AND PRIVATE TYPES WHOSE FULL DECLARATION " &
                     "IS AS AN INTEGER TYPE");

     CHECK_1 (-60, 9, "CHECK_INT");
     CHECK_2 (-60, 9, "DERIVED_INT");
     CHECK_4 (-60, 9, "INT_IN_P");
     CHECK_5 (-60, 9, "ALT_INT_IN_P");

     NEWPROC;

     IF CHECK_INT'SIZE /= MINIMUM_SIZE THEN
          FAILED ("CHECK_INT'SIZE INCORRECT");
     END IF;
