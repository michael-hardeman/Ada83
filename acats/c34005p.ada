...
PROCEDURE C34005P IS
     ...
     FUNCTION AGGR (X, Y : COMPONENT) RETURN ARRT IS
          RESULT : ARRT (INTEGER'FIRST .. INTEGER'FIRST + 1);
     BEGIN
          ASSIGN (RESULT (INTEGER'FIRST    ), X);
          ASSIGN (RESULT (INTEGER'FIRST + 1), Y);
          RETURN RESULT;
     END AGGR;

     FUNCTION AGGR (X, Y, Z : COMPONENT) RETURN ARRT IS
          RESULT : ARRT (INTEGER'FIRST .. INTEGER'FIRST + 2);
     BEGIN
          ASSIGN (RESULT (INTEGER'FIRST    ), X);
          ASSIGN (RESULT (INTEGER'FIRST + 1), Y);
          ASSIGN (RESULT (INTEGER'FIRST + 2), Z);
          RETURN RESULT;
     END AGGR;
...
BEGIN
     ...
     IF NOT EQUAL (T(U), AGGR (C1, C2, C3)) THEN
          FAILED ("INCORRECT CONVERSION FROM ARRAY");
     END IF;

     IF NOT EQUAL (ARR(X), AGGR (C1, C2, C3)) THEN
          FAILED ("INCORRECT CONVERSION TO ARRAY - 1");
     END IF;

     BEGIN
          IF NOT EQUAL (ARRT(CREATE (1, 2, C3, X)), AGGR (C3, C4)) THEN
               FAILED ("INCORRECT CONVERSION TO ARRAY - 2");
          END IF;
     EXCEPTION
          WHEN OTHERS =>
               FAILED ("EXCEPTION RAISED WHILE CHECKING BASE TYPE " &
                       "VALUES OUTSIDE OF THE SUBTYPE T - 2");
     END;
     ...
     IF T'BASE'SIZE /= PARENT'SIZE THEN
