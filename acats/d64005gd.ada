...
SEPARATE (D64005G0M.D64005GA.D64005GB.D64005GC)
...
BEGIN
     ...
     V (1) := IDENT_CHAR (ASCII.LC_D);
     V (2) := C;

     -- APPEND ALL V TO T.
     T.S (T.E+1 .. T.E+N) := D64005G0M.V & D64005GA.V & D64005GB.V &
                             D64005GC.V & D64005GD.V;
     T.E := T.E + N;

     CASE C IS

          WHEN '1' =>
               D64005GE (LEVEL'SUCC(L), IDENT_CHAR('1'), T);

          WHEN '2' =>
               D64005GD (L, IDENT_CHAR('3'), T);

          WHEN '3' =>
               D64005GE (LEVEL'SUCC(L), IDENT_CHAR('2'), T);
     END CASE;

     -- APPEND ALL L AND C TO T IN REVERSE ORDER.
     T.S (T.E+1 .. T.E+N) := D64005GD.L & D64005GD.C &
