...
-- HISTORY:
--     BCB 08/20/87  CREATED ORIGINAL TEST.
--     DHH 04/11/89  CHANGED EXTENSION FROM '.DEP' TO '.ADA', CHANGED
--                   SIZE CLAUSE VALUE TO 8, AND CHANGED OPERATOR ON
--                   'SIZE CHECKS.

WITH REPORT; USE REPORT;
PROCEDURE CD2A51B IS

     BASIC_SIZE : CONSTANT := 8;
     B : BOOLEAN;

BEGIN
     TEST ("CD2A51B", "CHECK THAT WHEN A SIZE SPECIFICATION IS " &
     ...
     BEGIN -- FUNC.
          ...
          IF CHARRAY(1)'SIZE < BASIC_SIZE THEN
          ...
          IF CHREC.COMPP'SIZE < BASIC_SIZE THEN
