...
SEPARATE (D64005F0M.D64005FA.D64005FB.D64005FC.D64005FD.D64005FE.
          D64005FF.D64005FG.D64005FH)

PROCEDURE D64005FI (L : LEVEL; C : CALL; T : IN OUT TRACE) IS
...
BEGIN
     ...
     CASE C IS
          ...
          WHEN '1' =>
               D64005FJ (LEVEL'SUCC(L), IDENT_CHAR('1'), T);

          WHEN '2' =>
               D64005FI (L, IDENT_CHAR('3'), T);

          WHEN '3' =>
               D64005FJ (LEVEL'SUCC(L), IDENT_CHAR('2'), T);
     END CASE;

     -- APPEND ALL L AND C TO T IN REVERSE ORDER.
     T.S (T.E+1 .. T.E+N) := D64005FI.L & D64005FI.C &
