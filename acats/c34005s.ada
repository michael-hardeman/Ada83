...
PROCEDURE C34005S IS
     ...
     FUNCTION AGGR (A, B, C, D, E, F : COMPONENT) RETURN ARRT IS
          X : ARRT (INTEGER'FIRST .. INTEGER'FIRST + 1,
                    INTEGER'FIRST .. INTEGER'FIRST + 2);
     BEGIN
          ASSIGN (X (INTEGER'FIRST    , INTEGER'FIRST    ), A);
          ASSIGN (X (INTEGER'FIRST    , INTEGER'FIRST + 1), B);
          ASSIGN (X (INTEGER'FIRST    , INTEGER'FIRST + 2), C);
          ASSIGN (X (INTEGER'FIRST + 1, INTEGER'FIRST    ), D);
          ASSIGN (X (INTEGER'FIRST + 1, INTEGER'FIRST + 1), E);
          ASSIGN (X (INTEGER'FIRST + 1, INTEGER'FIRST + 2), F);
          RETURN X;
     END AGGR;

     FUNCTION AGGR (A, B, C, D, E, F, G, H, I, J, K, L : COMPONENT)
                   RETURN ARRT IS
          X : ARRT (INTEGER'FIRST .. INTEGER'FIRST + 2,
                    INTEGER'FIRST .. INTEGER'FIRST + 3);
     BEGIN
          ASSIGN (X (INTEGER'FIRST    , INTEGER'FIRST    ), A);
          ASSIGN (X (INTEGER'FIRST    , INTEGER'FIRST + 1), B);
          ASSIGN (X (INTEGER'FIRST    , INTEGER'FIRST + 2), C);
          ASSIGN (X (INTEGER'FIRST    , INTEGER'FIRST + 3), D);
          ASSIGN (X (INTEGER'FIRST + 1, INTEGER'FIRST    ), E);
          ASSIGN (X (INTEGER'FIRST + 1, INTEGER'FIRST + 1), F);
          ASSIGN (X (INTEGER'FIRST + 1, INTEGER'FIRST + 2), G);
          ASSIGN (X (INTEGER'FIRST + 1, INTEGER'FIRST + 3), H);
          ASSIGN (X (INTEGER'FIRST + 2, INTEGER'FIRST    ), I);
          ASSIGN (X (INTEGER'FIRST + 2, INTEGER'FIRST + 1), J);
          ASSIGN (X (INTEGER'FIRST + 2, INTEGER'FIRST + 2), K);
          ASSIGN (X (INTEGER'FIRST + 2, INTEGER'FIRST + 3), L);
          RETURN X;
     END AGGR;
...
BEGIN
     ...
     IF NOT EQUAL (T (U), AGGR (C1, C2, C3, C4, C5, C6)) THEN
          FAILED ("INCORRECT CONVERSION FROM ARRAY");
     END IF;

     BEGIN
          IF NOT EQUAL (ARR (X), AGGR (C1, C2, C3, C4, C5, C6)) OR
             NOT EQUAL (ARRT (CREATE (7, 9, 2, 5, C3, X)),
                        AGGR (C3, C4, C5, C6, C7, C8, C9, C10, C11,
                              C12, C13, C14)) THEN
               FAILED ("INCORRECT CONVERSION TO ARRAY");
          END IF;
     EXCEPTION
          WHEN CONSTRAINT_ERROR =>
               FAILED ("CONSTRAINT_ERROR PREPARING FOR CONVERSION " &
                       "TO ARRAY");
          WHEN OTHERS =>
               FAILED ("EXCEPTION PREPARING FOR CONVERSION TO ARRAY");
     END;
     ...
     IF T'BASE'SIZE < (LAST - FIRST + 1) ** 2 * COMPONENT'SIZE THEN
