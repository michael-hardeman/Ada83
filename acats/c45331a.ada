...
C:   DECLARE
     ...
     BEGIN
          ...
          IF A_MIN + B_SMALL /= -3.9375 THEN
               FAILED ("-4.0 + 0.0625 /= -3.9375");
          END IF;

          IF A_MIN - B_SMALL /= -4.0625 THEN
               FAILED ("-4.0 - 0.0625 /= -4.0625");
          END IF;
          ...
          IF A_SMALL + B_MIN /= -0.3125 THEN
               FAILED ("0.5 + (-0.8125) /= -0.3125");
          END IF;

          IF A_SMALL - B_MIN /= 1.3125 THEN
               FAILED ("0.5 - (-0.8125) /= 1.3125");
          END IF;

          IF A_SMALL + B_SMALL /= 0.5625 THEN
               FAILED ("0.5 + 0.0625 /= 0.5625");
          END IF;

          IF A_SMALL - B_SMALL /= 0.4375 THEN
               FAILED ("0.5 - 0.0625 /= 0.4375");
          END IF;

          IF A_SMALL + B_MAX /= 5.5625 THEN
               FAILED ("0.5 + 5.0625 /= 5.5625");
          END IF;

          IF A_SMALL - B_MAX /= -4.5625 THEN
               FAILED ("0.5 - 5.0625 /= -4.5625");
          END IF;
          ...
          IF A_MAX + B_SMALL /= 3.0625 THEN
               FAILED ("3.0 + 0.0625 /= 3.0625");
          END IF;

          IF A_MAX - B_SMALL /= 2.9375 THEN
               FAILED ("3.0 - 0.0625 /= 2.9375");
          END IF;
          ...
          X := B_MIN - A_SMALL;
          IF X NOT IN -1.5 .. -1.25 THEN
               FAILED ("-0.8125 - 0.5 NOT IN RANGE");
          END IF;
          ...
          X := B_SMALL - A_MIN;
          IF X NOT IN 4.0 .. 4.25 THEN
               FAILED ("0.0625 - (-4.0) NOT IN RANGE");
          END IF;

          X := B_SMALL - A_SMALL;
          IF X NOT IN -0.5 .. -0.25 THEN
               FAILED ("0.0625 - 0.5 NOT IN RANGE");
          END IF;

          X := B_SMALL - A_MAX;
          IF X NOT IN -3.0 .. -2.75 THEN
               FAILED ("0.0625 - 3.0 NOT IN RANGE");
          END IF;
