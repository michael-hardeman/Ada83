...
SEPARATE (D64005G0M.D64005GA.D64005GB.D64005GC.D64005GD)

PROCEDURE D64005GE (L : LEVEL; C : CALL; T : IN OUT TRACE) IS
     ...
     PROCEDURE D64005GF (L : LEVEL; C : CALL; T : IN OUT TRACE) IS
          SEPARATE;
...
BEGIN
     ...
     CASE C IS
          ...
          WHEN '1' =>
               D64005GF (LEVEL'SUCC(L), IDENT_CHAR('1'), T);

          WHEN '2' =>
               D64005GE (L, IDENT_CHAR('3'), T);

          WHEN '3' =>
               D64005GF (LEVEL'SUCC(L), IDENT_CHAR('2'), T);
     END CASE;

     -- APPEND ALL L AND C TO T IN REVERSE ORDER.
     T.S (T.E+1 .. T.E+N) := D64005GE.L & D64005GE.C &
