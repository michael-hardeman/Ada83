...
PROCEDURE CD2A32A IS
     ...
     PROCEDURE CHECK_2 IS NEW LENGTH_CHECK (ARRAY_TYPE);
     PROCEDURE CHECK_3 IS NEW LENGTH_CHECK (REC_TYPE);
     ...
     BEGIN
          ...
          IF NOT (((PIN + PIP)   = PIOZ)       AND
                  ...
                  ((PIOZ / PIN)  = PIOZ)       AND
                  ((PIN ** 1)    = PIN)        AND
                  ((PIN REM 10)  = IDENT (-3)) AND
                  ((PIP MOD 10)  = IDENT (3))) THEN
          ...
          IF INT'POS (PIN)  /= IDENT_INT (-63) OR
             INT'POS (PIOZ) /= IDENT_INT (   0) OR
             INT'POS (PIP)  /= IDENT_INT ( 63) THEN
          ...
          IF INT'IMAGE (PIN)  /= IDENT_STR ("-63") OR
             INT'IMAGE (PIOZ) /= IDENT_STR (" 0")    OR
             INT'IMAGE (PIP)  /= IDENT_STR (" 63")  THEN
...
BEGIN
     ...
     CHECK_1 (I1, 7, "INT");
     CHECK_2 (INTARRAY, 21, "ARRAY_TYPE");
     CHECK_3 (IREC, 21, "REC_TYPE");
     ...
     IF INT'POS (INTARRAY (-1)) /= IDENT_INT (-63) OR
        INT'POS (INTARRAY ( 0)) /= IDENT_INT (   0) OR
        INT'POS (INTARRAY ( 1)) /= IDENT_INT ( 63) THEN
     ...
     IF INT'SUCC (INTARRAY (-1)) /= IDENT (-62) OR
     ...
     IF INT'IMAGE (INTARRAY (-1)) /= IDENT_STR ("-63") OR
        INT'IMAGE (INTARRAY ( 0)) /= IDENT_STR (" 0")    OR
        INT'IMAGE (INTARRAY ( 1)) /= IDENT_STR (" 63")  THEN
     ...
     IF INT'VAL (-63) /= IDENT (IREC.COMPN) OR
        INT'VAL (   0) /= IDENT (IREC.COMPZ) OR
        INT'VAL ( 63) /= IDENT (IREC.COMPP) THEN
     ...
     IF INT'PRED (IREC.COMPZ) /= IDENT (-1) OR
        INT'PRED (IREC.COMPP) /= IDENT (62) THEN
     ...
     IF INT'VALUE ("-63") /= IDENT (IREC.COMPN) OR
        INT'VALUE (   "0") /= IDENT (IREC.COMPZ) OR
        INT'VALUE ( "63") /= IDENT (IREC.COMPP) THEN
